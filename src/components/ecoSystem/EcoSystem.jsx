...
export default function EcoSystem() {
  ...
  const goToForm = () => {
    navigation("/ecoSystemForm")
  }


  const [data, setData] = useState([])
  useEffect(() => {
    axios.get(`https://rankterminal.com/growney/public/index.php/api/eco-system`)
      .then((response) => {
  ...
  const handelDelete = async (id) => {
    setIsLoading(true);
    try {
      const response = await fetch(`https://rankterminal.com/growney/public/index.php/api/eco-system/${id}`, {
        method: "DELETE",
      });
      if (!response.ok) {
          ...
          <div className='mx-auto mt-3 w-10/12 overflow-x-auto'>
            <table className='table-auto w-full overflow-x-auto'>
              <thead className='sticky top-0 border-b-2 border-b-white'>
                <tr>
                  <th className='sticky left-0 px-5 text-left bg-slate-600' style={{ minWidth: '180px' }}>Name</th>
                  <th className='text-left'>Socials</th>
                  <th className='px-5 text-left' style={{ minWidth: '200px' }}>Project</th>
